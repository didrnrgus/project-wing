...
def convert_to_utf8_bom(file_path):
    ...
    try:
        ...
        if encoding == 'utf-8-sig':  
            print(f"변환 불필요 (이미 UTF-8 BOM 포함): {file_path}")
            return
        ...
        print(f"변환 완료 ({encoding} → UTF-8 BOM): {file_path}")

    except Exception as e:
        print(f"변환 실패: {file_path} - 오류: {e}")

def process_directory():
    """디렉토리 내 .cpp, .h 파일을 UTF-8 with BOM으로 변환"""
...
if __name__ == "__main__":
    process_directory()
    print("모든 .cpp 및 .h 파일을 UTF-8 BOM 형식으로 변환 완료.")
